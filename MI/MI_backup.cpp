    ...
    for(int i=0; i<3800; i+=5){
        ...
        for (int i=0; i<result.rows; i++)
        {
            ...
            for (int j=0; j<result.cols; j++)
            {
                ...
                Mat src_hist = Mat::ones(1,45, CV_32FC1)*(1e-6);
        ...
        if(8>7){
            ...
            flag = imwrite(kitti360 + "MI_images/strange_images/"+keyWord+"_heatmap.png", heatMap, compression_params);
            flag = imwrite(kitti360 + "MI_images/strange_images/"+keyWord+"_seg_temp.png", ref_segrgb, compression_params);
            flag = imwrite(kitti360 + "MI_images/strange_images/"+keyWord+"_seg_match.png", src_segrgb, compression_params);
            // waitKey(0);
            exit(0);
        }
